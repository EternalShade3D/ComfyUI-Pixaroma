...
export const KEYWORDS = {
  ...
  PixaromaEdit3D: "3d model edit editor clean cleanup fix repair mesh sculpt brush select lasso panel piece move flatten smooth straighten sharpen crisp edge hard surface bevel delete fill hole holes close cracks seam inner shell double skin inside surfaces remove inside hollow floaters loose bits specks broken edges open edges quads quad remesh retopology topology reduce polygons decimate lighter make solid watertight 3d print printing printable slicer stl mirror make both sides match symmetry symmetric symmetrical both sides modes mode whole model polygons model check quick clean up one click x-ray wireframe pixal3d trellis hunyuan glb obj fbx ply blender game engine save export texture uv",
