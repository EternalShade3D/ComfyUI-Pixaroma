// Quad Remesh Pixaroma - wiring.
//
// core.mjs holds the state, ui.mjs the face, settings.mjs the gear panel, help.mjs the help page. The view
// draws through Load 3D's ONE shared renderer in its stage mode, the same way Save 3D's and Hard Surface's
// do (.claude/patterns/save-3d.md #6: stage mode must never change Load 3D). Adapted from
// js/hard_surface/index.js.
...
import { openQuadRemeshPanel, closeQuadRemeshPanelFor, isQuadRemeshPanelOpenFor } from "./settings.mjs";
import { QUAD_REMESH_HELP } from "./help.mjs";
...
registerNodeHelp(CLASS, QUAD_REMESH_HELP);
...
registerNodeSettings(CLASS, {
  ...
  closeFor: (node) => closeQuadRemeshPanelFor(node),
...
function toggleSettings(node) {
  if (isQuadRemeshPanelOpenFor(node)) closeQuadRemeshPanelFor(node);
  else openPanel(node);
}
...
/** Point the renderer at the file Before / Quads asks for. `fresh`: a run rewrote both under the same names. */
...
// an engine record. One shared check frees the records of Quad Remesh nodes that are neither in the graph
// nor on the page.
let _poll = 0;
...
app.registerExtension({
  name: "Pixaroma.QuadRemesh",

  beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData?.name !== CLASS) return;
    // A re-registration (hot reload) must not double-wrap every hook.
    if (nodeType.prototype._pixQrPatched) return;
    nodeType.prototype._pixQrPatched = true;
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      node._pixQrFloorOff = installResizeFloor(root, () => WIDGET_MIN_H);
      ...
      node._pixQrZoomOff = installZoomRepaint(node, null, () => requestDraw(node), "_pixQrRaf");
      try {
        node._pixQrRo = new ResizeObserver(() => requestDraw(node));
        node._pixQrRo.observe(node._pixQrEls.vp);
      } catch (_e) { /* no ResizeObserver: redraws still come from every change */ }
      attachCanvas(node, node._pixQrEls.canvas, () => renderFace(node), stageOptions());
      placeBand(node);
      node._pixQrRendererOff = onRendererChange(() => {
        placeBand(node);
        renderFace(node);
      });
    ...
    nodeType.prototype.onConnectionsChange = function () {
      ...
      const node = this;
      if (node._pixQrEls) queueMicrotask(() => renderFace(node));
      return r;
    };
    ...
    nodeType.prototype.onExecuted = function (message) {
      ...
      const report = message?.pixaroma_quadremesh?.[0];
    ...
    nodeType.prototype.onRemoved = function () {
      closeQuadRemeshPanelFor(this);
      this._pixQrFloorOff?.();
      this._pixQrFloorOff = null;
      this._pixQrZoomOff?.();
      this._pixQrZoomOff = null;
      this._pixQrRendererOff?.();
      this._pixQrRendererOff = null;
      try { this._pixQrRo?.disconnect(); } catch (_e) { /* already gone */ }
      this._pixQrRo = null;
      destroyFace(this);
      detach(this);
...
app.graphToPrompt = async function (...args) {
  ...
  try {
    ...
    console.error("[Pixaroma.QuadRemesh] inject failed", e);
  }
  return result;
};
